...
function getSlotIsoString(dateStr: string, timeSlotStr: string): string {
  // Combine the selected calendar day and time slot using your PHT operational offset
  const localDateTime = new Date(`${dateStr} ${timeSlotStr} UTC+8`);
...
export function BookingModal({ isOpen, onClose }: BookingModalProps) {
  ...
  // Synchronize internal mount state and manage global body overflow lock
  useEffect(() => {
    if (isOpen) {
      setIsMounted(true);
      setStep(1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Send all fields, selected date, and time slot to your server endpoint
      const response = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          dateStr: selectedDateStr, // e.g., "May 18, 2026"
          timeSlot: selectedSlot,   // e.g., "10:00 AM"
        }),
      });
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Failed to secure your booking.');
      }

      // 2. Success! Advance to Step 4 (Confirmation screen)
      setIsSubmitting(true);
      setStep(4);
    } catch (error: any) {
  ...
  // Prevent execution on server-side during Next.js SSR hydration
  ...
  // Inside your BookingModal Step 2 render tree:
  const [googleRawBusy, setGoogleRawBusy] = useState<{ start: string; end: string }[]>([]);

  // 1. Fetch the raw busy windows when Step 2 mounts
  useEffect(() => {
    if (step !== 2 || !selectedDateStr) return;

    async function fetchRawAvailability() {
      setIsLoadingSlots(true);
      try {
        const res = await fetch(`/api/bookings?date=${encodeURIComponent(selectedDateStr)}`);
        const data = await res.json();
        if (data.success && data.busy) {
          setGoogleRawBusy(data.busy);
        }
      } catch (err) {
        console.error("Failed fetching live timestamps:", err);
      } finally {
        setIsLoadingSlots(false);
      }
    }

    fetchRawAvailability();
  }, [step, selectedDateStr]);

  // 2. Compute true availability using clean 1-to-1 ISO comparisons
  const freeSlots = TIME_SLOTS.filter(slot => {
    if (!selectedDateStr) return false;

    // 1. Generate the exact 1-to-1 ISO string for the START of this slot (PHT / UTC+8)
    const slotStartLocal = new Date(`${selectedDateStr} ${slot} UTC+8`);
    const slotStartIso = slotStartLocal.toISOString();

    // 2. Generate the exact ISO string for the END of this slot (1 hour later)
    const slotEndLocal = new Date(slotStartLocal.getTime() + 60 * 60 * 1000); // Add 1 hour in milliseconds
    const slotEndIso = slotEndLocal.toISOString();

    // 3. Run a strict 1-to-1 overlap check against Google's busy blocks
    const isBusy = googleRawBusy.some(busyBlock => {
      // Standardize Google's timestamps to strict UTC ISO strings
      const busyStartIso = new Date(busyBlock.start).toISOString();
      const busyEndIso = new Date(busyBlock.end).toISOString();

      // A slot overlaps and is "busy" if:
      // The slot starts before the busy block ends AND the slot ends after the busy block starts
      const overlaps = slotStartIso < busyEndIso && slotEndIso > busyStartIso;

      return overlaps;
    });

    // Keep this slot only if it has zero overlaps with your calendar
    return !isBusy;
  });
      ...
      {isOpen && isMounted && (
        ...
        >
          <motion.div
            ...
            transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} // Match exact 'expo.out' physics
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-w-xl mx-auto">
                      {TIME_SLOTS.map(slot => {
                        const busy = !freeSlots.includes(slot);
                ...
                {step === 3 && (
                  ...
                  >
                    ...
                    <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-4">
                      ...
                      {/* Display a fallback error message block if the server errors out */}
                      {submitError && (
                        <div className="text-xs font-mono text-red-400 bg-red-500/10 border border-red-500/20 px-4 py-3 rounded-none">
                          Error // {submitError}
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className={`w-full py-4 bg-[#64FFDA] text-[#0A192F] font-bold text-xs uppercase tracking-[0.25em] transition-all duration-200 mt-2
                          ${isSubmitting ? 'opacity-50 cursor-wait tracking-widest' : 'hover:bg-[#4dffcc]'}
                        `}
                      >
                        {isSubmitting ? 'Processing_Secure_Link...' : 'Confirm Booking'}
                      </button>
                    </form>
                  </motion.div>
                )}
                  ...
                  >
                    ...
                    <div className="border border-[#64FFDA]/20 bg-[#64FFDA]/5 px-8 py-5 mb-10 text-left w-full max-w-xs space-y-2">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-[#F8F9FA] font-bold">30 Mins.</span> {/* Adjusted to match your 1-hour slot layout */}
                      </div>
                      <div className="flex justify-between text-xs">
