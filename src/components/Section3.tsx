    ...
    <section id="process" ref={containerRef} data-scroll-anim className="relative h-screen bg-[#0A192F] overflow-hidden">
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-20 flex gap-4">
        {[0, 1, 2].map((step) => (
          <div 
            key={step} 
            className={`h-1 w-12 rounded transition-colors duration-300 ${activeStep === step ? 'bg-[#F8F9FA]' : 'bg-[#8892B0]/30'}`}
          />
        ))}
      </div>
        ...
        <div className="process-panel w-screen h-full flex items-center justify-center px-12 md:px-24">
          <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <a href="https://calendar.google.com" target="_blank" rel="noreferrer" className="inline-block border border-[#F8F9FA] text-[#F8F9FA] hover:bg-[#F8F9FA] hover:text-[#0A192F] px-6 py-3 text-sm font-medium transition-colors">
