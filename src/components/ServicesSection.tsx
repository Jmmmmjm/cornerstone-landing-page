  ...
  return (
    <div ref={containerRef} className="relative h-[400vh] bg-transparent">
      ...
      >
        ...
        <div className="w-full lg:w-1/2 h-full flex flex-col justify-center pt-20 lg:pt-0 px-8 lg:px-12 z-10 text-right bg-transparent">
          <Reveal>
            <div className="font-mono text-sm text-black/40 dark:text-[#F8F9FA]/40 tracking-widest uppercase mb-4">
              We are setting
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <h3 className="text-4xl md:text-5xl lg:text-[4.4vw] font-display font-bold uppercase tracking-tight text-[#0A192F] dark:text-[#F8F9FA] leading-[1.0] ml-auto max-w-2xl">
        ...
        <div className="w-full lg:w-[44%] h-full flex flex-col justify-start mr-auto relative">
          <motion.div 
            style={{ y }}
            className="flex flex-col gap-12 pt-24 pb-24 px-8 lg:px-12"
          >
            {services.map((service, i) => (
              <div key={i} className="flex flex-col w-full">
                {i > 0 && (
                  <div className="w-[85%] mx-auto h-px bg-black/10 dark:bg-[#F8F9FA]/10 mb-12" />
                )}
                <div className="bg-transparent min-h-[480px] p-8 md:p-12 flex flex-col justify-between group">
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2 mb-8">
                       <span className="font-display text-2xl font-bold text-teal-600 dark:text-[#64FFDA]">{service.number}</span>
                       <h4 className="text-2xl md:text-3xl font-display font-bold uppercase text-[#0A192F] dark:text-[#F8F9FA]">
                         {service.title}
                       </h4>
                    </div>
                    
                    <div className="flex items-center justify-center py-4">
                       <div className="w-32 h-32 border border-teal-500/20 flex items-center justify-center">
                          <div className="w-20 h-20 border-2 border-dashed border-teal-500/40 animate-[spin_20s_linear_infinite]" />
                       </div>
                    </div>
                  </div>

                  <p className="text-base text-[#0A192F]/70 dark:text-[#F8F9FA]/60 leading-relaxed font-sans">
                    {service.description}
                  </p>
                </div>
              </div>
