    ...
    <div ref={containerRef} className="absolute inset-0 bg-[#F8F9FA] dark:bg-[#0A192F] overflow-hidden">
      ...
      <LegacyWindow title="Internal Mail" className="absolute top-[5%] left-[2%] w-[300px] h-[250px] opacity-40 -rotate-3">
        <div className="space-y-3">
          <div className="h-2 bg-gray-200 rounded-none w-full"></div>
          <div className="h-2 bg-gray-200 rounded-none w-5/6"></div>
          <div className="h-2 bg-gray-100 rounded-none w-4/6"></div>
          <div className="h-12 bg-blue-50 border border-blue-100 rounded-none"></div>
        </div>
      </LegacyWindow>
      ...
      <LegacyWindow title="Q4_Final_Final_v2.xlsx" className="absolute top-[10%] right-[2%] w-[350px] h-[300px] opacity-40 rotate-6">
        <div className="grid grid-cols-4 gap-1">
          {[...Array(24)].map((_, i) => (
            <div key={i} className={`h-3 rounded-none ${i % 7 === 0 ? 'bg-red-100' : 'bg-gray-100'}`}></div>
          ))}
        </div>
      </LegacyWindow>
      ...
      <LegacyWindow title="Manual Override" className="absolute bottom-[15%] right-[8%] w-[280px] h-[180px] opacity-50 -rotate-2">
        <div className="flex flex-col items-center justify-center h-full gap-2">
          <div className="w-10 h-10 rounded-none border-4 border-red-200 border-t-red-500"></div>
          ...
          <div className="legacy-window w-full h-[500px] bg-[#F8F9FA] border border-gray-400 rounded-none overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="w-48 bg-[#F3F4F6] border-r border-gray-300 p-4 flex flex-col gap-2">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="h-6 bg-gray-200 rounded-none border border-gray-300 flex items-center px-2">
                    <div className="w-2 h-2 bg-gray-400 rounded-none mr-2"></div>
                    <div className="h-2 bg-gray-300 rounded-none w-full"></div>
                  </div>
                ))}
              </div>
              <div className="flex-1 p-6 bg-[#F8F9FA] overflow-hidden">
                <div className="flex gap-4 mb-6">
                  <div className="flex-1 h-32 bg-red-50 border border-red-200 rounded-none p-4">
                    ...
                    <div className="h-2 bg-red-200 rounded-none w-3/4 mb-2"></div>
                    <div className="h-2 bg-red-100 rounded-none w-full"></div>
                  </div>
                  <div className="w-1/3 h-32 bg-gray-50 border border-gray-200 rounded-none p-4">
                    <div className="text-[10px] font-bold text-gray-400 uppercase mb-2">Idle Resource</div>
                    <div className="h-2 bg-gray-200 rounded-none w-1/2"></div>
                  </div>
                </div>
                <div className="space-y-2">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="h-8 bg-gray-50 border border-gray-200 rounded-none flex items-center px-4 gap-4">
                      <div className="w-4 h-4 bg-gray-200 rounded-none"></div>
                      <div className="h-2 bg-gray-200 rounded-none flex-1"></div>
                      <div className="h-2 bg-gray-200 rounded-none w-24"></div>
                    </div>
                  ))}
                </div>
              </div>
